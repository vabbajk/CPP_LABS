...
TransactionList Database::loadTransactionList() {
    ...
    while (file.good() && !file.eof()) {
        // Проверяем, можем ли прочитать хотя бы тип транзакции
        ...
        // Возвращаемся назад для полного чтения
        file.seekg(pos);
    ...
    // Обновляем nextID, чтобы новые транзакции получали правильные ID
    if (maxID > 0) {
