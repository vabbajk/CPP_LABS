...
bool Database::writeBytes(const std::byte* data, std::size_t size) {
    // std::fstream requires char* interface, reinterpret_cast is necessary
    if (!file.write(reinterpret_cast<const char*>(data), static_cast<std::streamsize>(size))) { // NOSONAR
        return false;
    }
    return file.good();
}
...
void Database::readBytes(std::byte* data, std::size_t size) {
    // std::fstream requires char* interface, reinterpret_cast is necessary
    file.read(reinterpret_cast<char*>(data), static_cast<std::streamsize>(size)); // NOSONAR
}
