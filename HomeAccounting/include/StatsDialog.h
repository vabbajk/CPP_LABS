#pragma once

#include <QDialog>
#include <QDateEdit>
#include <QVBoxLayout>
#include <QHBoxLayout>
#include <QLabel>
#include <QScrollArea>
#include <QPainter>
#include <QMap>
#include <QPushButton>
#include "TransactionList.h"

class PieChartWidget : public QWidget {
    Q_OBJECT
public:
    explicit PieChartWidget(QWidget* parent = nullptr) : QWidget(parent) {}

    void setData(const QMap<QString, double>& categoryToValue, const QString& title) {
        data = categoryToValue;
        chartTitle = title;
        update();
    }

protected:
    void paintEvent(QPaintEvent* event) override {
        QWidget::paintEvent(event);
        QPainter p(this);
        p.setRenderHint(QPainter::Antialiasing, true);

        QRect rect = this->rect().adjusted(20, 20, -20, -20);
        // Title with modern styling
        QFont f = p.font();
        f.setBold(true);
        f.setPointSize(13);
        p.setFont(f);
        p.setPen(QColor(88,166,255));  // Vibrant blue for title
        p.drawText(rect.left(), rect.top(), rect.width(), 30, Qt::AlignLeft | Qt::AlignVCenter, chartTitle);

        QRect pieRect = QRect(rect.left(), rect.top() + 45, rect.height() - 50, rect.height() - 50);
        if (pieRect.width() > rect.width()/2) {
            int size = qMin(rect.width()/2 - 20, pieRect.height());
            pieRect.setWidth(size);
            pieRect.setHeight(size);
        }

        double total = 0.0;
        for (auto it = data.constBegin(); it != data.constEnd(); ++it) total += it.value();
        if (total <= 0.0) {
            p.setPen(QColor(139,148,158));  // Muted text
            QFont emptyFont = p.font();
            emptyFont.setPointSize(12);
            emptyFont.setBold(false);
            p.setFont(emptyFont);
            p.drawText(rect, Qt::AlignCenter, QString::fromUtf8("📭 Нет данных за выбранный период"));
            return;
        }

        // Modern vibrant colors palette for dark theme
        QList<QColor> colors = {
            QColor(88,166,255),   // Blue
            QColor(121,192,255),  // Light Blue
            QColor(248,81,73),    // Red
            QColor(255,123,114),  // Light Red
            QColor(63,185,80),    // Green
            QColor(87,242,135),   // Light Green
            QColor(255,171,64),   // Orange
            QColor(255,203,107),  // Light Orange
            QColor(187,128,255),  // Purple
            QColor(214,175,255)   // Light Purple
        };

        int colorIdx = 0;
        int startAngle = 0;
        QList<QPair<QString,double>> items;
        for (auto it = data.constBegin(); it != data.constEnd(); ++it) items.append({it.key(), it.value()});
        std::sort(items.begin(), items.end(), [](auto& a, auto& b){ return a.second > b.second; });

        for (const auto& item : items) {
            double value = item.second;
            int span = static_cast<int>((value / total) * 360.0 * 16);
            QColor c = colors[colorIdx % colors.size()];
            p.setBrush(c);
            p.setPen(QPen(QColor(33,38,45), 2));  // Dark border for better separation
            p.drawPie(pieRect, startAngle, span);
            startAngle += span;
            colorIdx++;
        }

        // Legend with modern styling
        int legendX = pieRect.right() + 30;
        int legendY = pieRect.top();
        colorIdx = 0;
        QFont legendFont = p.font();
        legendFont.setPointSize(10);
        legendFont.setBold(false);
        p.setFont(legendFont);
        
        for (const auto& item : items) {
            QColor c = colors[colorIdx % colors.size()];
            p.setBrush(c);
            p.setPen(Qt::NoPen);
            QRect box(legendX, legendY, 18, 18);
            p.drawRoundedRect(box, 5, 5);  // Rounded legend boxes
            
            p.setPen(QColor(230,237,243));  // Light text for dark theme
            double percent = (item.second / total) * 100.0;
            QString text = QString("%1 — %2 руб. (%3%)").arg(item.first)
                .arg(QString::number(item.second, 'f', 2))
                .arg(QString::number(percent, 'f', 1));
            p.drawText(legendX + 28, legendY + 14, text);
            legendY += 28;
            colorIdx++;
        }
    }

private:
    QMap<QString, double> data;
    QString chartTitle;
};

class StatsDialog : public QDialog {
    Q_OBJECT
public:
    explicit StatsDialog(const TransactionList* list, QWidget* parent = nullptr);
    ~StatsDialog() override = default;

private:
    const TransactionList* transactionList;
    QDateEdit* dateFrom;
    QDateEdit* dateTo;
    PieChartWidget* incomeChart;
    PieChartWidget* expenseChart;

    void recalc();
};





